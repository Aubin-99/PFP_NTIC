    ...
    if (!isLogged) {
      ...
      AuthenticationService.getCurrent()
        .then((response) => {
          ...
          EducatorService.getUsersByEducator(response.data.id)
            .then((educatorUsers) => {
    ...
    if (!isLogged) {
      try {
        ...
        await AsyncStorage.setItem(
          "@can_delete_fav_picto",
          JSON.stringify(user.CanDeleteFavPicto)
        );
        await AsyncStorage.setItem(
          "@can_delete_fav_sentence",
          JSON.stringify(user.CanDeleteFavSentence)
        );
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.userItem}>
                ...
                <PermissionButton
                  isEnabled={item.CanDeleteFavPicto}
                  handlePress={() => handlePermissionChange("pictogram", item)}
                  type={"pictogram"}
                />
                <PermissionButton
                  isEnabled={item.CanDeleteFavSentence}
                  handlePress={() => handlePermissionChange("sentence", item)}
                  type={"sentence"}
                />
              </View>
            )}
        ...
        <View>
          {usersLeft.length !== 0 && (
            <View>
              ...
              <TouchableOpacity
                style={styles.adminButton}
                onPress={addUserToEducator}
              >
                <Text style={styles.buttonText}>
                  {"Prendre en charge l'utilisateur"}
                </Text>
              </TouchableOpacity>
            </View>
          )}
